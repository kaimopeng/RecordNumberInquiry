...
def icp(company):
    try:
        burp0_url = "https://www.110icp.com:443/index.php?keyword={}&s=beian&c=search".format(company)
        html = requests.get(burp0_url).text
        soup = BeautifulSoup(html, 'html.parser')
...
def beianx(company):
    try:
        ...
        burp0_cookies = {"__51huid__JfwpT3IBSwA9n8PZ": "a500d976-04bb-5a56-bd03-d8eb49e67781", "__51vcke__JfvlrnUmvss1wiTZ": "58630161-395e-534c-aa0f-a34d1aa4fce1", "__51vuft__JfvlrnUmvss1wiTZ": "1695104138587", "__51uvsct__JfvlrnUmvss1wiTZ": "5"}
        burp0_headers = {"Sec-Ch-Ua": "\"Chromium\";v=\"117\", \"Not;A=Brand\";v=\"8\"", "Sec-Ch-Ua-Mobile": "?0", "Sec-Ch-Ua-Platform": "\"macOS\"", "Upgrade-Insecure-Requests": "1", "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/117.0.5938.63 Safari/537.36", "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7", "Sec-Fetch-Site": "none", "Sec-Fetch-Mode": "navigate", "Sec-Fetch-User": "?1", "Sec-Fetch-Dest": "document", "Accept-Encoding": "gzip, deflate, br", "Accept-Language": "zh-CN,zh;q=0.9", "Connection": "close"}
        html = requests.get(burp0_url, headers=burp0_headers, cookies=burp0_cookies).text
        soup = BeautifulSoup(html, 'html.parser')
        table = str(soup.find('table', class_='table table-sm table-bordered table-hover'))
        beian1 = re.findall(r'(.ICP[备|证]\d{7,10}号)',table)
        beian2 = re.findall(r'(.B2-\d{8})',table)
        beian = beian1 + beian2
        ba = list(set(beian))
...
if __name__ == '__main__':
    ...
    with open('company.txt', 'r') as beiancofile:
        for companys in beiancofile:
            ...
            print(company)
            if chinaz(company) is not None:
                if icplishi(company) is not None:
                    if icp(company) is not None:
                        if beianx(company) is not None:
                            if tycbeian(company) is not None:
                                cookie = get_cookies()
                                    ...
                                    if cookie != -1:
                                        ...
                                        if token != -1:
                                            ...
                                            if check_data != -1:
                                                ...
                                                if sign != -1:
                                                    get_beian_info(info, p_uuid, token, sign)       
                                except:
                                    pass
    print(list_beian)
